#!/usr/bin/env node
/**
 * Keeps a single GitHub Release (`apk`) with one asset: NLC.apk.
...
if (!apkPath || !fs.existsSync(apkPath)) {
  console.error("Uso: node scripts/publish-github-apk.mjs <NLC.apk> <version> <versionCode> [notas]");
  process.exit(1);
}

const title = `NLC ${version} (${versionCode})`;
const body = [
  `APK completa **${version}** · versionCode **${versionCode}**.`,
  "",
  "Esta release **solo guarda la última APK** (`NLC.apk`). El código sigue versionado en `main`.",
  "",
  "Instálala cuando el cambio sea nativo (plugins, permisos, Expo SDK, notificaciones…). El JS cotidiano va por OTA.",
  "",
  notes ? notes : "",
  "",
  "Descarga directa: https://github.com/JuanCarlosGP/NLC/releases/download/apk/NLC.apk",
]
...
if (releaseExists()) {
  ...
  for (const asset of assets) {
    if (asset.name && asset.name !== "NLC.apk") {
      gh(["release", "delete-asset", "apk", asset.name, "--yes"]);
    }
  }
  gh(["release", "upload", "apk", `${apkPath}#NLC.apk`, "--clobber"]);
  gh(["release", "edit", "apk", "--title", title, "--notes", body, "--latest"]);
} else {
  gh(["release", "create", "apk", `${apkPath}#NLC.apk`, "--title", title, "--notes", body, "--latest"]);
}

console.log(`GitHub Release actualizado: ${title}`);
console.log("https://github.com/JuanCarlosGP/NLC/releases/download/apk/NLC.apk");
