...
Small Docker service used by **SND**. You do **not** install Python on UGOS — only Docker / Container Manager.

Requires a Ugreen **DXP** (or other host) with Docker. DH series without Docker: run this compose on a PC instead; point SND at that host.
...
3. Edit `docker-compose.yml`:
   ...
   - Set `AUTH_TOKEN` (same value you will enter in SND).
...
- **Viewer** only needs **read** on `Music` / `Podcasts` so SND WebDAV can play the new files.
- Write on Viewer is only needed for `snd.json` (Guardar configuración), not for podcast downloads.
...
Do not use Spotify episode links for audio download (DRM / unsupported). Spotify import in SND remains metadata-only.
